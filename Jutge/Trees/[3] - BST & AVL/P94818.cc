...
class Abin {
  private:
    ...
    // Pre: cert
    // Post: Retorna true si l’element k est`a en el p.i. tenint en compte que  ́es un BST
    static void print(node *n, int nivell);
  ...
  public:
    ...
    // Pre: cert
    // Post: L’element k s’ha inserit en el p.i. tenint en compte que  ́es un BST
    void print() const;
};
...
template< typename T>
void Abin<T>:: print(node *n, int nivell) {
	if(n != nullptr){
		print(n->f_dret, nivell+1);
		cout << setw(nivell*10)<< n->info<< endl;
		print(n->f_esq, nivell+1);
	}
